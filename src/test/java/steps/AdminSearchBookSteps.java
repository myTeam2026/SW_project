...
public class AdminSearchBookSteps {
    ...
    public AdminSearchBookSteps(ServiceSteps serviceSteps) {
        this.serviceSteps = serviceSteps;
    }
    
   public AdminSearchBookSteps() {
    ...
    @When("the user searches for a book with title {string}")
    public void searchBookByTitle(String title) {
        List<Book> results = BookData.searchBooksByTitle(title);
        serviceSteps.setSearchResults(results);
    }

    @When("the user searches for a book with author {string}")
    public void searchBookByAuthor(String author) {
        List<Book> results = BookData.searchBooksByAuthor(author);
        serviceSteps.setSearchResults(results);
    }
    ...
    public void searchBookByISBN(String isbn) {
        Book book = BookData.getBookByISBN(isbn);
        serviceSteps.setSearchResults(book != null ? List.of(book) : List.of());
    }

    @Then("the search results should include a book with title {string}")
    public void verifyBookByTitle(String title) {
        boolean exists = serviceSteps.getSearchResults().stream()
                .anyMatch(b -> b.getTitle().equalsIgnoreCase(title));
        Assert.assertTrue("Book with title should exist", exists);
    }

    @Then("the search results should include a book with author {string}")
    public void verifyBookByAuthor(String author) {
        boolean exists = serviceSteps.getSearchResults().stream()
                .anyMatch(b -> b.getAuthor().equalsIgnoreCase(author));
        Assert.assertTrue("Book with author should exist", exists);
    }
    ...
    public void verifyBookByISBN(String isbn) {
        ...
        Assert.assertTrue("Book with ISBN should exist", exists);
    }
}
